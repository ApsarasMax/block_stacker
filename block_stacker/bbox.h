...
private:
	bool checkIntersect(Vec3f const& L, BoundingBox const& rhs){
	    ...
	    float dotProduct=D*L;
	    float sum1=abs(this->a1*this->vec1*L)+abs(this->a2*this->vec2*L)+abs(this->a3*this->vec3*L);
	    float sum2=abs(rhs.a1*rhs.vec1*L)+abs(rhs.a2*rhs.vec2*L)+abs(rhs.a3*rhs.vec3*L);
